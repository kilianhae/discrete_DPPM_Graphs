...
# Simpler EMD for 1D histograms NOTE: fix for different supports
# def emd(x, y, axis=-1):
#     n = x.shape[0]
#     p = y.shape[axis]
#     c1 = np.cumsum(x, axis=axis).reshape([n ,-1])/p
#     c2 = np.cumsum(y, axis=axis).reshape([n ,-1])/p
    
#     delta = c1.mean(0) - c2.mean(0)
