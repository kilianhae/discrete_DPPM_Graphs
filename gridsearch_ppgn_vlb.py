...
# Creates new directories for the configs of our runs, for the slurm_job numbers, for the slurm_scripts to run based on the model parameters chosen and based on the timestamp
...
with open('config/gridsearch_ppgn_final.yaml') as f:
    ...
    for batchsize, dataset, networksize, noisetype, weighted_loss, seed in itertools.product(batchsizes, datasets, networksizes, noisetypes, weighted_losses,seeds):
        ...
        if sigmarandom == False:
            batchsize = noise_nums[0]
        ...
        with open(f'config/gridsearch/{testdir}/gridsearch_ppgn_consec_{dataset}_{networksize[0]},{networksize[1]}_{len(noise_nums)}_{batchsize}_{noisetype}_{weighted_loss}_{seed}.yaml',"w+") as g:
            yaml.dump(data, g)
        commandstring = f"python3 ppgn_vlb.py -c config/gridsearch/{testdir}/gridsearch_ppgn_consec_{dataset}_{networksize[0]},{networksize[1]}_{len(noise_nums)}_{batchsize}_{noisetype}_{weighted_loss}_{seed}.yaml"
        with open('scripts/gridsearch.sh','r') as firstfile, open(f"scripts/gridsearch/{testdir}/gridsearch_ppgn_consec_{dataset}_{networksize[0]},{networksize[1]}_{len(noise_nums)}_{batchsize}_{noisetype}_{weighted_loss}_{seed}.sh",'a+') as secondfile:
        ...
        out = subprocess.check_output(f"sbatch scripts/gridsearch/{testdir}/gridsearch_ppgn_consec_{dataset}_{networksize[0]},{networksize[1]}_{len(noise_nums)}_{batchsize}_{noisetype}_{weighted_loss}_{seed}.sh",shell=True)
        index = str(out).find("\n")
        jobnumber = int(out[index-6:index])
        with open(f"gridsearch/{testdir}/gridsearch_ppgn_consec_{dataset}_{networksize[0]},{networksize[1]}_{len(noise_nums)}_{batchsize}_{noisetype}_{weighted_loss}_{seed}.txt","w+") as idfile:
