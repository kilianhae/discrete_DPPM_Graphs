...
##tries to get score list as close as possible to grad_log_q_noise_list which is 1 for flipped and 0 for noneflipped
        ...
        for epoch in range(max_epoch):#range(max_epoch):
            ...
            try:      
                if epoch%config.finalinterval==config.finalinterval-1 and config.eval_from<epoch:
                    with torch.no_grad():
                        wandb_dict={}
                        results=sample_main_edp(config,f"{config.model_save_dir}/best",epoch,num_noiselevel)
                        wandb_dict.update({f"degree_mmd_{num_noiselevel}_best": results["degree"],f"cluster_mmd_{num_noiselevel}_best": results["cluster"],f"orbit_mmd_{num_noiselevel}_best": results["orbit"],f"testloss_best": best_score})
                        wandb.log(wandb_dict)
            except:
                print("error")
            ...
            try:      
                if epoch%config.finalinterval==config.finalinterval-1 and config.eval_from<epoch:
                    with torch.no_grad():
                        wandb_dict={}
                        results=sample_main_edp(config,f"{config.model_save_dir}/bestloss",epoch,num_noiselevel)
                        wandb_dict.update({f"degree_mmd_{num_noiselevel}_bestloss": results["degree"],f"cluster_mmd_{num_noiselevel}_bestloss": results["cluster"],f"orbit_mmd_{num_noiselevel}_bestloss": results["orbit"],f"testloss_bestloss": best_score})
                        wandb.log(wandb_dict)
            except:
                print("error")
