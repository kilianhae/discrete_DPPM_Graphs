...
def load_data_sbm(config, get_graph_list=False):
    graph_list_unclean = torch.load("./data/"+config.dataset.name+".pt")[:-5][0]
    graph_list=[]
    for i in range(len(graph_list_unclean)):
        if graph_list_unclean[i].size(0)>config.dataset.max_node_num:
            continue
        else:
            graph_list.append(graph_list_unclean[i])

    x_tensor=torch.zeros(len(graph_list),config.dataset.max_node_num)
    for i in range(len(graph_list)):
        graph_list[i]=torch.tensor(pad_adjs(graph_list[i],node_number=config.dataset.max_node_num),dtype=torch.float32)
    
    config.dataset.in_feature = 1
    ...
    if get_graph_list:
        return train_graph_list, test_graph_list
    x_tensor_train, x_tensor_test = x_tensor[test_size:], x_tensor[:test_size]
    
    train_ds_train = TensorDataset(torch.stack(train_graph_list,dim=0),x_tensor_train)
    train_dl_train = DataLoader(train_ds_train, batch_size=config.train.batch_size, shuffle=True)
    train_ds_test = TensorDataset(torch.stack(test_graph_list,dim=0),x_tensor_test)
    train_dl_test = DataLoader(train_ds_test, batch_size=config.train.batch_size, shuffle=True)
    
    return train_dl_train, train_dl_test
